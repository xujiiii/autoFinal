...
def main():
    ...
    ap.add_argument("--shap-top-csv", required=True, type=Path, help="lgbm_shap_top.csv")
    ap.add_argument("--conserved-csv", required=True, type=Path, help="conserved map file")
    ap.add_argument("--manifest-csv", required=True, type=Path, help="manifest from pipeline 1")
    ap.add_argument("--full-pdb-dir", required=True, type=Path)
    ap.add_argument("--out", required=True, type=Path)
    ap.add_argument("--dfg-col", type=str, default="dfg_spatial", help="column name in manifest to represent DFG status")
    args = ap.parse_args()
    ...
    if args.dfg_col not in manifest.columns:
        raise KeyError(f"No '{args.dfg_col}' found in {args.manifest_csv}. All columns: {list(manifest.columns)}")
    ...
    for ii in range(n_samples):
        if ii % 200 == 0:
            print(f"Compute PDB distance: {ii}/{n_samples}")
    ...
    for tg in ("z0", "z1"):
        ...
        print(f"Save {tg} (DFGin vs DFGout) plot and CSV file")

    print(f"All output under: {args.out}")
